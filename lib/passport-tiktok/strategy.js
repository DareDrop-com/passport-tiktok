...
Strategy.prototype.userProfile = function (accessToken, authData, done) {
  ...
  this._oauth2.get(`${url}?${parsedParams}`, accessToken, function (err, body, res) {
    ...
    function getUsernameFromProfileUrl(url) {
      ...
      if (!usernameMatch) {
        console.info(TIKTOK_STRATEGY_PREFIX, 'failed to find username in url', url);
        return undefined;
      }
    ...
    try {
      ...
      const profileShortLink = `https://open-api.tiktok.com/shortlink/profile/?open_id=${json.data.user.open_id}`
      return _fetch(profileShortLink, {
        headers: {
          /*
           The issue is probably deeper,
           check:
           - https://stackoverflow.com/questions/74596976/making-get-request-to-a-tiktok-url-in-order-to-get-a-canonical-link/74597310
           - https://stackoverflow.com/questions/23013220/max-retries-exceeded-with-url-in-requests
           I accidentally found a solution that works locally and on lambda by passing profile link,
           but seems like this part is enough
           some of User-Agent will work locally but not on lambda, so it makes it harder to debug,
           TODO: remove it when TikTok api will return username in user info response
           */
          'User-Agent': 'https://open',
        },
        timeout: 10000,
      })
        .then(response => {
          ...
          var profile = {
            ...
            username: getUsernameFromProfileUrl(profileUrl),
        ...
        .catch(err => {
          ...
          console.log(TIKTOK_STRATEGY_PREFIX, json, profileShortLink)
