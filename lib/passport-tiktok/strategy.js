...
Strategy.prototype.userProfile = function (accessToken, authData, done) {
  ...
  this._oauth2.get(`${url}?${parsedParams}`, accessToken, function (err, body, res) {
    ...
    try {
      ...
      return _fetch(user.profile_deep_link, {
        timeout: 10000,
      })
        .then(response => {
          ...
          if (!username) {
            ...
            console.info(TIKTOK_STRATEGY_PREFIX, response.url)
        ...
        .catch(err => {
          console.info(TIKTOK_STRATEGY_PREFIX, err)
          console.info(TIKTOK_STRATEGY_PREFIX, json)
