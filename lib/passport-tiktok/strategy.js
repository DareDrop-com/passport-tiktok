...
const TIKTOK_STRATEGY_PREFIX = '[tiktok strategy]'
...
Strategy.prototype.userProfile = function (accessToken, authData, done) {
  ...
  const params = {
    ...
    fields: ['open_id', 'union_id', 'avatar_url', 'avatar_url_100', 'avatar_url_200', 'avatar_large_url', 'display_name', 'profile_deep_link'],
  };
  console.info(TIKTOK_STRATEGY_PREFIX, 'request for profile', params.open_id)
  this._oauth2._request('POST', url, {}, JSON.stringify(params), accessToken, function (err, body, res) {
    ...
    function getUsernameFromProfileUrl(url) {
      ...
      if (!match) {
        console.info(TIKTOK_STRATEGY_PREFIX, 'username not found', url)
        return undefined;
      }
    ...
    try {
      ...
      console.info(TIKTOK_STRATEGY_PREFIX, 'processing tiktok profile response')
      
      console.info(TIKTOK_STRATEGY_PREFIX, 'json response', json)
      
      // Api returns shortened URL, that's why we need here two requests
      self._oauth2.get(json.data.user.profile_deep_link, null, function (_error, _buffer, incomingShortenedUrlMessage) {
      console.info(TIKTOK_STRATEGY_PREFIX, '_error _buffer', _error, _buffer)
        console.info(TIKTOK_STRATEGY_PREFIX, 'incomingShortenedUrlMessage', incomingShortenedUrlMessage)
        const location = incomingShortenedUrlMessage.headers.location;
        console.info(TIKTOK_STRATEGY_PREFIX, 'request to', location)
        try {
          self._oauth2.get(location, null, function (error, buffer, incomingProfileUrlMessage) {
            console.info(TIKTOK_STRATEGY_PREFIX, 'incomingProfileUrlMessage', incomingProfileUrlMessage)
            console.info(TIKTOK_STRATEGY_PREFIX, 'error', 'buffer', error, buffer)
            try {
              var profile = {
                provider: 'tiktok',
                id: json.data.user.open_id,
                unionId: json.data.user.union_id,
                username: getUsernameFromProfileUrl(incomingProfileUrlMessage.headers.location),
                profileImage: json.data.user.avatar_url_100,
                displayName: json.data.user.display_name,
              };
  
              console.log(TIKTOK_STRATEGY_PREFIX, 'successfully got profile', profile)
              ...
              console.info(TIKTOK_STRATEGY_PREFIX, 'Failed to retrieve profile username')
              console.info(TIKTOK_STRATEGY_PREFIX, 'profile response', json)
              console.info(TIKTOK_STRATEGY_PREFIX, 'incomingProfileUrlMessage', incomingProfileUrlMessage);
              console.info(TIKTOK_STRATEGY_PREFIX, 'incomingShortenedUrlMessage', incomingShortenedUrlMessage);
              console.error(TIKTOK_STRATEGY_PREFIX, e)
              console.error(TIKTOK_STRATEGY_PREFIX, error)
              done(new Error('Username for this account could not be retrieved'))
            }
          });
        } catch (e) {
          // Failed to get username
          console.info(TIKTOK_STRATEGY_PREFIX, 'Failed to get username')
          console.info(TIKTOK_STRATEGY_PREFIX, 'incomingShortenedUrlMessage', incomingShortenedUrlMessage)
          console.info(TIKTOK_STRATEGY_PREFIX, 'profile response', json)
          console.error(TIKTOK_STRATEGY_PREFIX, e)
          done(new Error('User profile link could not be fetched'))
        }
      });
    } catch (e) {
  ...
  if (req.query && req.query.code) {
    function loaded(err, ok, state) {
      ...
      self._oauth2._request('POST', tokenUrl, null, null, null, (err, data) => {
          ...
          if (!params || !params.data || !params.data.access_token) {
...
Strategy.prototype._loadUserProfile = function (accessToken, paramsData, done) {
  ...
  console.info(TIKTOK_STRATEGY_PREFIX, 'loadUserProfile')
